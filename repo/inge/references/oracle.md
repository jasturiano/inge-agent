...
Group independent consequential questions into a small, numbered round with
recommendations. Hold dependent questions until their prerequisites are answered;
continue unrelated investigation meanwhile. Avoid a fresh interview for an already
accepted ticket. The completion criterion is resolved blocking decisions, not every
imaginable branch explored. Routine technical choices stay with the implementer.

Challenge overloaded domain terms against the project's glossary and concrete
edge cases. Keep accepted terminology distinct from implementation details. When
the user cannot answer, establish who can and what decision is needed; prepare a
focused questionnaire only when requested. Do not send it automatically. Propose an
ADR when the decision involves a consequential tradeoff that future readers would
otherwise misunderstand; save only within the authorized scope.
