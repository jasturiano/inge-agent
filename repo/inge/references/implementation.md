...
Before changing anything: confirm scope/acceptance criteria, inspect current changes
using permitted native tools or user-supplied Git status/diff evidence, and preserve
the user's prior work. The shipped OpenCode profile denies direct Git status/diff
commands; never wrap or delegate them to bypass that denial.
Capture relevant checks as a baseline
...
Use debugging.md for reproduction strategies and verification.md for application
recipes. Review consequential failure and security paths with security.md. Load
only what the actual task needs; imported skills do not force additional ceremonies.
...
Except approved stash push/pop under WORKFLOW.md, Git mutations are prohibited. Suggest them for manual
execution only, including commits, pulls, merges and worktree operations. Never run
verification scripts that create commits or otherwise mutate Git fixtures.
Deleting the only copy, migrations,
...
Both routine and strong writing roles follow this reference and WORKFLOW.md.
