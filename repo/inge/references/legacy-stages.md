# Numbered RPI/recon compatibility through /inge

The original recovery kit was inspected. Its useful stage contracts are
consolidated below, not restored as sixteen competing slash commands. The router must
recognize `/inge <old-stage-name> <arguments>` as well as the equivalent plain-English
request. These are request forms inside the real `/inge` command, not additional
OpenCode commands or a script engine. Load only the referenced procedure and needed
artifacts. Execute through the active adapter's permitted tools and specialists;
printing a slash command is not execution.
...
| Original stage / arguments after /inge | Inputs | Behavior and output |
|---|---|---|
...
For new scenarios, `/inge Draft acceptance scenarios for <ID>` uses planning.md and
...
`/inge Close <ID>` uses continuity.md and updates 06-closure.md for a numbered ticket.
...
| Original stage / arguments after /inge | Inputs | Behavior and saved output |
|---|---|---|
...
A new `/inge Architecture review ...` uses compact A0-A5 report sections instead.
`/inge Draft quality scenarios for <SYSTEM>` uses A0 priorities and records the
...
| Original stage / arguments after /inge | Input and output contract |
|---|---|
...
`/inge Synthesize recon for <ID>` saves recon/03-architecture.md from inventory/flows.
`/inge Contrast recon for <ID> with <declared-source>` saves recon/04-drift.md from
